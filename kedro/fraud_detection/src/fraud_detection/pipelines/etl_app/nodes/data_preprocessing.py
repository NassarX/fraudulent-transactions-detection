...
def split_transaction_data_into_batches(
    transactions_data: pd.DataFrame, start_date: str
) -> Dict[str, Any]:
    ...
    for day in range(transactions_data.TX_TIME_DAYS.max() + 1):
        transactions_per_day = transactions_data[
...
def process_daily_transaction_data(
    ...
    for partition_id, partition_load_func in sorted(partitioned_input.items()):
        ...
        merged_df = pd.concat(
            [merged_df, partition_data], ignore_index=True, sort=True

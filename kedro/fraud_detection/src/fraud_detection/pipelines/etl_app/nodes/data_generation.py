...
def generate_terminals_data(n_terminals: int, random_state=1) -> pd.DataFrame:
    ...
    terminal_data = pd.DataFrame(
        terminal_properties, columns=["TERMINAL_ID", "x_terminal_id", "y_terminal_id"]
    )

    return terminal_data


def select_terminals_within_customer_radius(
    customer_profile: dict, terminals_data: pd.DataFrame, radius: float
) -> list:
    ...
    x_y_terminals = terminals_data[["x_terminal_id", "y_terminal_id"]].values.astype(
        float
    )

    # Get customer coordinates
    x_y_customer = np.array(
        [customer_profile["x_customer_id"], customer_profile["y_customer_id"]],
        dtype=float,
    )
...
def map_terminals_to_customers(
    customers_data: pd.DataFrame, terminals_data: pd.DataFrame, radius: float
) -> pd.DataFrame:
    ...
    customers_data["available_terminals"] = customers_data.apply(
        lambda customer: select_terminals_within_customer_radius(
            customer, terminals_data, radius
        ),
        axis=1,
    )
...
def generate_transactions_data(
    customers_terminals_data: pd.DataFrame, start_date: str, nb_days: int
) -> pd.DataFrame:
    ...
    for _, customer_profile in customers_terminals_data.groupby("CUSTOMER_ID"):
        ...
        available_terminals = ast.literal_eval(
            customer_profile["available_terminals"].values[0]
        )
        if nb_terminals == 0:
                ...
                for tx in range(nb_tx):
                    ...
                    if 0 < time_tx < 86400:
                        ...
                        if nb_terminals > 0:
                            ...
                            transaction = pd.DataFrame(
                                {
                                    "TX_DATETIME": pd.to_datetime(
                                        tx_time_seconds, unit="s", origin=start_date
                                    ),
                            ...
                            transactions_data = pd.concat(
                                [transactions_data, transaction], ignore_index=True
                            )
...
def generate_fraud_Scenarios_data(
    ...
    for day in range(transactions_data["TX_TIME_DAYS"].max()):
        compromised_terminals = terminals_data["TERMINAL_ID"].sample(
            n=2, random_state=day
        )

        compromised_transactions = transactions_data[
    ...
    for day in range(transactions_data["TX_TIME_DAYS"].max()):
        compromised_customers = (
            customer_profiles_data["CUSTOMER_ID"].sample(n=3, random_state=day).values
        )
        ...
        transactions_data.loc[index_frauds, "TX_AMOUNT"] = (
            transactions_data.loc[index_frauds, "TX_AMOUNT"] * 5
        )
        transactions_data.loc[index_frauds, "TX_FRAUD"] = 1
        transactions_data.loc[index_frauds, "TX_FRAUD_SCENARIO"] = 3

    nb_frauds_scenario_3 = (
        transactions_data["TX_FRAUD"].sum()
...
def generate_dataset(
    ...
    customer_profiles_data = generate_customer_profiles_data(
    ...
    customers_terminals_data = map_terminals_to_customers(
        customer_profiles_data, terminals_data, radius
    )

    # Generate transactions data
    transactions_data = generate_transactions_data(
        customers_terminals_data, start_date, nb_days
    )
